...
# my own functions
from GMSAT_classes import ModelInput, Gradient
...
def GenerateRN(timeseries, timeseriesMean, timeseriesStd):
    '''
    Function that takes in a time series (e.g., temperature anomalies) and generates a red noise time series of the same length with the same AR1 coefficient as the input time series; and the same
    mean and std
    
    Inputs:
        timeseries: time series data (e.g., temperature); note that is a dataarray normally; note that this input needs a 'year' dimension
        
    Outputs:
        Red noise time series of the same length
    '''
    numberPoints = len(timeseries.values)

    # defining a function to calculate the red noise time series
    # setting 0 intercept in the red noise time series
    # finding the AR1 as the correlation between the values and the values at lag 1
    AR1, pVal = pearsonr(timeseries.values[:numberPoints-1], timeseries.values[1:numberPoints])

    # initialising the RN time series
    redNoise = np.zeros(numberPoints)
    redNoise[0] = timeseriesMean

    # generating a WN time series
    whiteNoiseRN = np.random.normal(1, 1, numberPoints)

    # calculating the RN time series
    for t in range(1, numberPoints):
        redNoise[t] = AR1*redNoise[t-1] + whiteNoiseRN[t]

    # now normalising the redNoise to have the same std as the original timeseries
    redNoise = (redNoise - redNoise.mean()) / redNoise.std()
    redNoise = redNoise * timeseriesStd + timeseriesMean
    
    # saving this as an xarray dataarray
    years = timeseries.year
    
    redNoiseDa = xr.DataArray(redNoise,
                   dims = ['year'],
                   coords = {'year': years})
    
    return redNoiseDa

def GenerateWN(timeseries, timeseriesMean, timeseriesStd):
    '''
    Function that takes in a time series (e.g., temperature) and generates a white noise time series of the same length with the same mean and std
    
    Inputs:
        timeseries: time series data (e.g., temperature); note that is a dataarray normally; note that this input needs a 'year' dimension
        
    Outputs:
        White noise time series of the same length
    '''
    numberPoints = len(timeseries.values)
    
    whiteNoise = np.random.normal(timeseriesMean, timeseriesStd, numberPoints)
    
     # saving this as an xarray dataarray
    years = timeseries.year
    
    whiteNoiseDa = xr.DataArray(whiteNoise,
                   dims = ['year'],
                   coords = {'year': years})
